...
import io.neoterm.shared.errors.Error;
import io.neoterm.shared.file.FileUtils;
import io.neoterm.shared.logger.Logger;
import io.neoterm.shared.neoterm.file.NeoTermFileUtils;
...
public class CameraPhotoAPI {
    ...
    public static void onReceive(NeoTermApiReceiver apiReceiver, final Context context, Intent intent) {
        ...
        ResultReturner.returnData(apiReceiver, intent, stdout -> {
            ...
            String photoFilePath = NeoTermFileUtils.getCanonicalPath(filePath, null, true);
            ...
            // under allowed neoterm working directory paths.
            // We try to set execute permissions, but ignore if they are missing, since only read and write permissions are required
            // for working directories.
            Error error = NeoTermFileUtils.validateDirectoryFileExistenceAndPermissions("photo directory", photoDirPath,
                    true, true, true,
                    false, true);
            if (error != null) {
