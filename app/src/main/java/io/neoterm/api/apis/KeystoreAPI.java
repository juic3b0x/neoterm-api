...
import io.neoterm.api.util.ResultReturner.ResultJsonWriter;
import io.neoterm.api.util.ResultReturner.WithInput;
...
public class KeystoreAPI {
    ...
    @SuppressLint("NewApi")
    public static void onReceive(NeoTermApiReceiver apiReceiver, Intent intent) {
    ...
    private static void listKeys(NeoTermApiReceiver apiReceiver, final Intent intent) {
     ...
     * </ul>
     */
    private static void deleteKey(NeoTermApiReceiver apiReceiver, final Intent intent) {
    ...
    private static void generateKey(NeoTermApiReceiver apiReceiver, final Intent intent) {
        ResultReturner.returnData(apiReceiver, intent, out -> {
            String alias = intent.getStringExtra("alias");
     ...
     * </ul>
     */
    private static void signData(NeoTermApiReceiver apiReceiver, final Intent intent) {
     ...
     * </ul>
     */
    private static void verifyData(NeoTermApiReceiver apiReceiver, final Intent intent) {
        ResultReturner.returnData(apiReceiver, intent, new WithInput() {
            @Override
    ...
    private static void printErrorMessage(NeoTermApiReceiver apiReceiver, Intent intent) {
        ResultReturner.returnData(apiReceiver, intent, out -> out.println("neoterm-keystore requires at least Android 6.0 (Marshmallow)."));
    }
}
